...
def make_ppo_lstm_policy(env, config):
    ...
    # NOTE: "MlpLstmPolicy" is not a valid policy for stable_baselines3.PPO
    # (recurrent policies live in sb3-contrib's RecurrentPPO). The recurrence
    # here is supplied by the custom LstmFeaturesExtractor below, so the
    # standard feed-forward "MlpPolicy" is the correct choice.
    model = PPO(
        "MlpPolicy",
        env,
